...
<dialog bind:this={dialog} class="conversion-dialog" oncancel={(event) => { event.preventDefault(); finish(null); }}>
  <form class="stack" onsubmit={(event) => { event.preventDefault(); if (password) finish(password); }}>
    <div><h2>Confirm your password</h2><p class="muted">Sensitive owner actions require a recent password confirmation.</p></div>
