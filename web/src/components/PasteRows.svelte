<script lang="ts">
  ...
  async function copyLink(paste: Paste): Promise<void> {
    await navigator.clipboard.writeText(
      new URL(paste.url ?? `/pastes/${paste.id}`, location.origin).href
    );
  ...
  <div class:admin-paste-table={context === "admin"}>
    {#if context === "admin"}
      <div class="admin-paste-head" aria-hidden="true">
        <span
          >Paste{#if totalItems !== undefined}<small>{totalItems} total</small>{/if}</span
        >
    ...
    >
      {#each items as paste, index (paste.id)}
        <article
          ...
          class:selectable
          class:admin-paste-row={context === "admin"}
        >
          {#if context === "admin"}
            <div class="paste-row-identity">
              <Link class="paste-title" href={`/pastes/${paste.id}`}
                >{pasteDisplayTitle(paste)}</Link
              >
            ...
            <div class="admin-paste-owner">
              ...
              {:else}
                <Link href={filterUrl("owner_id", String(paste.owner_id))}>
                  <strong
                    >{paste.owner_username ??
                      ownerNames?.get(paste.owner_id) ??
                      `User #${paste.owner_id}`}</strong
                  >
          ...
          {:else}
            {#if selectable}<input
                class="paste-selector"
                type="checkbox"
                aria-label={`Select ${pasteDisplayTitle(paste)}`}
                aria-describedby="paste-range-selection-help"
                title="Shift-click to select a range"
                checked={selected.has(paste.id)}
                onclick={(event) => selectPaste(index, event.currentTarget.checked, event.shiftKey)}
              />{/if}
            <div class="paste-row-identity">
              <Link class="paste-title" href={`/pastes/${paste.id}`}
                >{pasteDisplayTitle(paste)}</Link
