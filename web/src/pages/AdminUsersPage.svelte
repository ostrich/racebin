<script lang="ts">
  ...
  $effect(() => {
    ...
    loading = true;
    void listAdminUsers(source).then(value => {
      if (current !== generation) return;
      page = value;
      search = query.get("search") ?? "";
      error = "";
    }).catch(reason => {
      if (current === generation) error = reason instanceof Error ? reason.message : "Unable to load users";
    }).finally(() => {
...
<section class="page-layout" aria-busy={loading}>
  ...
  <form class="list-filter-bar admin-user-filters" onsubmit={applySearch}>
    ...
    <label class="field list-filter-select"><span>Role</span><select value={query.get("role") ?? ""} onchange={event => setFilter("role", event.currentTarget.value)}><option value="">Any role</option><option value="user">User</option><option value="admin">Administrator</option><option value="owner">Owner</option></select></label>
    ...
    <label class="field list-filter-select"><span>Sort</span><select value={query.get("sort") ?? "username"} onchange={event => setFilter("sort", event.currentTarget.value)}><option value="username">Username</option><option value="created">Newest</option><option value="login">Last login</option><option value="pastes">Paste count</option><option value="storage">Storage</option></select></label>
    <label class="field list-filter-select"><span>Direction</span><select value={query.get("direction") ?? ((query.get("sort") ?? "username") === "username" ? "asc" : "desc")} onchange={event => setFilter("direction", event.currentTarget.value)}><option value="asc">Ascending</option><option value="desc">Descending</option></select></label>
  </form>
  {#if error}<section class="empty"><h2>Unable to load users</h2><p>{error}</p></section>
  {:else if page}<p class="result-count">{page.total_items} users</p><div class="panel admin-user-table" role="table" aria-label="Users">
    ...
    {#each page.items as user (user.id)}
      <div class="admin-user-row" role="row">
        ...
        <div class="badge-group"><span class="badge">{user.role === "owner" ? "Owner" : user.role === "admin" ? "Administrator" : "User"}</span><span class:danger={!user.enabled} class="badge">{user.enabled ? "Enabled" : "Disabled"}</span></div>
  ...
  </div><Pagination {page} params={query}/>{:else}<p class="muted">Loading users…</p>{/if}
  </div></div>
</section>
