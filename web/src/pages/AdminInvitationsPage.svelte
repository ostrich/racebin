<script lang="ts">
  ...
  const statusLabel = (status: Invitation["status"]) => status.charAt(0).toUpperCase() + status.slice(1);
  ...
  <div class="section-layout">
    ...
    <div class="section-content invitation-management">
      ...
      <nav class="section-tabs" aria-label="Invitation records">
        <Link href={tabUrl("active")} aria-current={view === "active" ? "page" : undefined}>Active <span>{activeCount}</span></Link>
      ...
      {:else if page}
        <div class="panel data-list invitation-list">
          {#each page.items as invitation (invitation.id)}
            <article class="data-row invitation-row">
              <div class="invitation-identity">
                ...
                <small>Created by {invitation.created_by_username} · {formatDate(invitation.created_at)}</small>
              </div>
              <div class="invitation-lifecycle">
                <span class:danger={invitation.status === "expired" || invitation.status === "revoked"} class="badge">{statusLabel(invitation.status)}</span>
                <small>{invitation.redeemed_by_username ? `Redeemed by ${invitation.redeemed_by_username}${invitation.redeemed_at ? ` · ${formatDate(invitation.redeemed_at)}` : ""}` : `Expires ${formatDate(invitation.expires_at)}`}</small>
          ...
          {:else}
            <div class="empty"><p>{view === "active" ? "No active invitations." : "No invitation history matches these filters."}</p></div>
          {/each}
        </div>
