<script lang="ts">
  ...
  onMount(() => {
    void load()
      .catch((reason) => {
        error = reason instanceof Error ? reason.message : "Unable to load user";
      })
  ...
  async function patch(values: UserUpdate): Promise<void> {
    ...
    try {
      ...
      showNotice(reason instanceof Error ? reason.message : "Unable to update account", "error");
      await load();
    } finally {
      busy = false;
    }
  ...
  async function resetLink(): Promise<void> {
    ...
    try {
      ...
      showNotice(reason instanceof Error ? reason.message : "Unable to create reset link", "error");
    }
  }
  ...
  async function revoke(kind: "sessions" | "api-keys", label: string): Promise<void> {
    if (
      ...
      !(await confirmAction({
        title: `${label}?`,
        message: `Apply this action to ${user.username}?`,
        confirmLabel: label,
        dangerous: true
      }))
    )
    ...
    try {
      ...
      showNotice(
        reason instanceof Error ? reason.message : `Unable to ${label.toLowerCase()}`,
        "error"
      );
    }
  }
  ...
  async function toggleEnabled(): Promise<void> {
    ...
    if (
      ...
      !(await confirmAction({
        ...
        message: "The account will be disabled and all of its sessions will be revoked.",
  ...
  async function confirmPassword(): Promise<boolean> {
    ...
    try {
      ...
      showNotice(
        reason instanceof Error ? reason.message : "Password confirmation failed",
        "error"
      );
      return false;
    }
  ...
  async function saveRole(): Promise<void> {
    ...
    try {
      ...
      showNotice(reason instanceof Error ? reason.message : "Unable to update role", "error");
    }
  }
  ...
  async function makeOwner(): Promise<void> {
    if (
      ...
      !(await confirmAction({
        ...
        message:
          "This account will become the site owner. Your account will remain an administrator.",
    ...
    try {
      ...
      showNotice(
        reason instanceof Error ? reason.message : "Unable to transfer ownership",
        "error"
      );
    }
  }
...
<section class="page-layout">
  {#if error}<section class="empty">
      ...
      <p>{error}</p>
      <Link class="button" href="/admin/users">Back to users</Link>
    </section>
  ...
  {:else}
    <div class="page-heading">
      ...
      <div class="page-heading-actions">
        <Link class="button" href={`/admin/pastes?owner_id=${user.id}`}>View pastes</Link>
      </div>
    </div>
    <div class="section-layout">
      ...
      <div class="section-content">
        <div class="admin-user-metrics">
          <article class="panel">
            <span>Pastes</span><strong>{user.paste_count}</strong><small
              >{formatByteSize(user.storage_bytes)} stored</small
            >
          </article>
          <article class="panel">
            <span>Sessions</span><strong>{user.active_session_count}</strong><small>active</small>
          </article>
          <article class="panel">
            <span>API keys</span><strong>{user.active_api_key_count}</strong><small
              >{user.api_key_count} total</small
            >
          </article>
          <article class="panel">
            <span>Last login</span><strong class="metric-date"
              >{user.last_login_at ? formatDate(user.last_login_at) : "Never"}</strong
        ...
        <div class="admin-user-panels">
          <section class="panel stack">
            ...
            <p class="muted">Control whether this account can sign in.</p>
            {#if $appState.session.user?.role === "owner" && user.role !== "owner"}<div
                class="admin-user-access"
              >
                <label class="field"
            ...
            <button
              ...
              onclick={toggleEnabled}>{user.enabled ? "Disable account" : "Enable account"}</button
            >
            {#if $appState.session.user?.role === "owner" && user.role === "admin"}<button
                class="button danger"
                type="button"
                onclick={makeOwner}>Transfer ownership to this administrator</button
              >{/if}
          </section>
          <section class="panel stack">
            <h2>Security</h2>
            <p class="muted">
              Recovery links are valid once for one hour. Resetting a password signs out every
              existing session.
            </p>
            <div class="admin-security-actions">
              <button class="button" type="button" disabled={!canManage} onclick={resetLink}
                ><Icon name="link-2" /> Create and copy reset link</button
              ><button
                class="button"
                type="button"
                disabled={!canManage}
                onclick={() => revoke("sessions", "Sign out all sessions")}
                >Sign out everywhere</button
              ><button
                class="button danger"
                type="button"
                disabled={!canManage}
                onclick={() => revoke("api-keys", "Revoke all API keys")}
