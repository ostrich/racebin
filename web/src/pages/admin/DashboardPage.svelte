<script lang="ts">
  ...
  const invitationsUnavailable = $derived(
    isOwner && !$appState.config.invitations_enabled && (summary?.active_invitation_count ?? 0) > 0
  );
  const actionCount = $derived(
    passwordResets + expiringInvitations + (invitationsUnavailable ? 1 : 0)
  );
  ...
  onMount(() => {
    const requests: [Promise<AdminSummary>, Promise<AdminUser[]>, Promise<AuditEvent[]>] = [
      getAdminSummary(),
      listAdminUsers(
        new URLSearchParams({ sort: "created", direction: "desc", page_size: "5" })
      ).then((page) => page.items),
      isOwner
        ? listAuditEvents(new URLSearchParams({ page_size: "5" })).then((page) => page.items)
        : Promise.resolve([])
    ];
    void Promise.all(requests)
      ...
      .catch((reason) => {
        error = reason instanceof Error ? reason.message : "Unable to load administration overview";
      })
    ...
    <div class="section-content admin-dashboard">
      ...
      {:else}
        <section class="admin-summary" aria-label="Site summary">
          <article class="panel">
            <span>Users</span><strong>{summary?.user_count ?? 0}</strong>
          </article>
          <article class="panel">
            <span>Pastes</span><strong>{summary?.paste_count ?? 0}</strong>
          </article>
          <article class="panel">
            <span>Stored data</span><strong>{formatByteSize(summary?.storage_bytes ?? 0)}</strong>
          </article>
          <article class="panel">
            <span>Active sessions</span><strong>{summary?.active_session_count ?? 0}</strong>
          </article>
        </section>

        <section class="panel dashboard-section">
          <div class="section-heading">
            <div><h2>Needs action</h2></div>
            {#if actionCount}<span class="badge danger">{actionCount}</span>{/if}
          </div>
          {#if actionCount}
            <div class="data-list compact-data-list">
              {#if passwordResets}<Link href="/admin/users" class="data-row"
                  ><span>Password changes required</span><strong>{passwordResets}</strong></Link
                >{/if}
              {#if expiringInvitations}<Link href="/admin/invitations" class="data-row"
                  ><span>Invitations expiring within four hours</span><strong
                    >{expiringInvitations}</strong
                  ></Link
                >{/if}
              {#if invitationsUnavailable}<Link href="/admin/settings" class="data-row"
                  ><span>Active invitations cannot be redeemed while invitations are disabled</span
                  ><strong>Review</strong></Link
                >{/if}
            </div>
          {:else}
            <p class="dashboard-status">
              <Icon name="check" /> No administrative follow-up is needed.
            </p>
          {/if}
        </section>
        ...
        <section class="panel dashboard-section">
          <div class="section-heading">
            <div><h2>{isOwner ? "Recent activity" : "Recent accounts"}</h2></div>
          </div>
          <div class="data-list compact-data-list">
            {#if isOwner}
              {#each events.slice(0, 5) as event (event.id)}
                {@const destination = activityUrl(event)}
                {#if destination}<Link href={destination} class="data-row"
                    ><div>
                      <strong>{activityLabel(event.action)}</strong><small
            ...
            {:else}
              {#each recentUsers as user (user.id)}
                <Link class="data-row" href={`/admin/users/${user.id}`}
                  ><div>
                    <strong>{user.username}</strong><small
                      >Joined {formatDate(user.created_at)}</small
                    >
                  </div>
                  <span class="badge">{user.role === "admin" ? "Administrator" : "User"}</span
                  ></Link
                >
        ...
        <section class="panel dashboard-section">
          ...
          <div class="dashboard-actions">
            {#if $appState.config.invitations_enabled}<button
                class="button primary"
                type="button"
                onclick={() => invitationDialog.open()}
