<script lang="ts">
  ...
  let canManage = $derived(Boolean(paste?.source_url));
  let showWrapOption = $derived(
    Boolean(
      (paste?.format === "text" || markdownView === "markdown") && (horizontalOverflow || wrapLines)
    )
  );
  let printLanguage = $derived(
    paste
      ? ($appState.languages.find((language) => language.id === paste?.language)?.label ??
          paste.language)
      : ""
  );
...
{#if paste}
  <article class="paste-view page-layout">
    <div class="page-heading">
      <div>
        ...
        <p class="paste-print-metadata">
          <span
            ...
            {paste.visibility.charAt(0).toUpperCase() + paste.visibility.slice(1)}</span
          >
      ...
      <div class="actions page-heading-actions">
        ...
        {#if paste.raw_url}<a
            ...
            title="Raw"
            aria-label="Raw"><Icon name="file-code" /></a
          >{/if}
        <button
          class="icon-button"
          type="button"
          title="Copy"
          aria-label="Copy"
          onclick={copyContent}><Icon name="copy" /></button
        >
        <button
          class="icon-button"
          type="button"
          title={preparingPrint ? "Preparing print" : "Print"}
          aria-label={preparingPrint ? "Preparing print" : "Print"}
          disabled={preparingPrint}
          onclick={printPaste}><Icon name="printer" /></button
        >
        {#if paste.archive_url}<a
            class="icon-button"
            href={paste.archive_url}
            title="Download ZIP"
            aria-label="Download ZIP"><Icon name="archive" /></a
          >{/if}
        {#if $appState.config.qr_codes_enabled}<a
            class="icon-button"
            href={pasteQrUrl($appState.config.api_base_url ?? "/api/v1", paste.id)}
            title="QR code"
            aria-label="QR code"><Icon name="qr-code" /></a
          >{/if}
        {#if canManage}<Link
            class="icon-button primary"
            href={`/pastes/${paste.id}/edit`}
            title="Edit"
            aria-label="Edit"><Icon name="edit-3" /></Link
          >{/if}
        {#if canManage}<button
            class="icon-button danger"
            type="button"
            title={deleting ? "Deleting paste" : "Delete"}
            aria-label={deleting ? "Deleting paste" : "Delete"}
            disabled={deleting}
            onclick={removePaste}><Icon name="trash-2" /></button
          >{/if}
      </div>
    </div>
    ...
    {:else}
      <CodeViewer
        bind:this={codeViewer}
        code={paste.content}
        language={paste.format === "markdown" ? "markdown" : paste.language}
        wrap={wrapLines}
...
{:else if error}
  <section class="empty">
    <h1>Unable to load this paste</h1>
    <p>{error}</p>
