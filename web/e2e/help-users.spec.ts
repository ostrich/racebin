...
test("signed-in help explains API keys using the current installation", async ({ page }) => {
  ...
  await expect(
    page.getByText("Authorization: Bearer $RACEBIN_API_KEY", { exact: false }).first()
  ).toBeVisible();
...
test("site basics explains creation, visibility, limits, and management", async ({ page }) => {
  ...
  await expect(
    basics.getByRole("heading", { name: "Control how long it remains available" })
  ).toBeVisible();
  await expect(basics.getByRole("heading", { name: "Organize and manage" })).toBeVisible();
  await expect(
    basics.getByRole("heading", { name: "Before sharing sensitive information" })
  ).toBeVisible();
});
...
test("administrator can inspect a user and copy a recovery link", async ({ page }) => {
  ...
  await expect(page.locator(".admin-paste-head > span")).toHaveText([
    /Paste\s*1 total/,
    "Owner",
    "Details",
    "Actions"
