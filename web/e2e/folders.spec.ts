...
test("folders can be searched, created, renamed, and deleted from the picker", async ({ page }) => {
  ...
  const createRequest = page.waitForRequest(
    (request) => request.url().endsWith("/api/v1/folders") && request.method() === "POST"
  );
  await page.getByRole("button", { name: /^Scripts/ }).click();
  ...
  const renameRequest = page.waitForRequest(
    (request) => request.url().endsWith("/api/v1/folders/5") && request.method() === "PATCH"
  );
  await page.getByRole("button", { name: /^Scripts/ }).click();
  ...
  await expect(
    page
      .getByRole("dialog", { name: "Browse folders" })
      .getByRole("button", { name: /^Utilities 1$/ })
  ).toBeVisible();

  const deleteRequest = page.waitForRequest(
    (request) => request.url().endsWith("/api/v1/folders/5") && request.method() === "DELETE"
  );
...
test("mobile folder and filter controls do not overflow", async ({ page }) => {
  ...
  const layout = await page.evaluate(() => ({
    ...
    searchColumns: getComputedStyle(
      document.querySelector(".paste-search")!
