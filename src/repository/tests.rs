...
mod tests {
    ...
    async fn legacy_rich_documents_migrate_without_losing_dependent_rows() {
        let data_dir = std::env::temp_dir().join(format!("racebin-markdown-migration-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&data_dir).unwrap();
        let url = format!("sqlite://{}?mode=rwc", data_dir.join("database.sqlite").display());
        let repository = Repository::open(&url, &data_dir).await.unwrap();
        ...
        repository.migrate().await.unwrap();
        let migrated: (String, String, i64) = sqlx::query_as("SELECT content_kind,content,revision FROM pastes WHERE id='legacy'").fetch_one(repository.pool()).await.unwrap();
