...
mod tests {
    ...
    use crate::pastes::Permission;

    #[test]
    fn owner_permissions_are_a_strict_session_only_superset() {
        let session = |is_owner| {
            Principal::Session(SessionUser {
                user: User {
                    id: 1,
        ...
        assert!(session(false).allows(Permission::ManageUsers));
        assert!(!session(false).allows(Permission::ConfigureInstance));
        ...
        let key = Principal::ApiKey(ApiKey {
            ...
            user_id: Some(1),
            name: "admin".into(),
            token_prefix: "key".into(),
            scopes: vec!["user:manage".into(), "instance:configure".into()],
            created_at: 0,
            last_used_at: None,
            enabled: true,
        });
        assert!(key.allows(Permission::ManageUsers));
        assert!(!key.allows(Permission::ConfigureInstance));
    }
