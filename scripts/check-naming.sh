...
if find . \
    \( -path './.git' -o -path './target' -o -path './web/dist' \
       -o -path './web/node_modules' -o -path './packaging/arch/src' \
       -o -path './packaging/arch/pkg' \) -prune -o \
    -type f ! -path './scripts/check-naming.sh' \
    -exec grep -InHEi "$pattern" {} +
then
